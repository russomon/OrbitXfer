...
## v0.1.84 - 2026-06-07 (phase 4, polish — UNRELEASED)
- **Pair flow polish.** The Shareable Code feature now has feature-parity with the classic Send/Receive on the high-value bits Phase 3 deferred.
  - **Transfer-complete summary cards** on both sides. Same layout as the classic Send/Receive summary (Name / Size / Speed / Time + Saved-to on the receiver) so the eye knows where to look. Times bracket only the actual data-transfer window — for pair-host the clock starts at the first `upload_started`, for pair-join it starts at `pairing_download_started` (so the ~1 s Argon2id+CPace handshake doesn't pollute the speed reading).
  - **Resume-last** for both pair flows. Sender: `↻ Re-share by code: <filename>` button re-shares the same source file with a brand-new code (codes are always single-use). Receiver: `↻ Resume last code receive: <code>` retries the last code+destination, useful for a quick retry after a hiccup if the sender's pair-host is still running. Both persist via the same `localStorage` + cross-window-storage-event pattern Send/Receive use; the new keys are `orbitxfer.lastPairHost.v1` and `orbitxfer.lastPairJoin.v1`.
  - **Live code-input validation** on the receiver. As the user types, a one-line indicator shows `✓ 4 words` (green) or `n of 4 words` (muted) with the normalized form rendered inline. The Receive button is gated on the word count — no more accidental clicks with 3 words or 5. We don't validate dictionary membership in the frontend (would bundle 7,776 EFF words for ~30 KB of payload); a wrong word's worst case is a clean iroh-discovery miss with no information leakage.
  - **`pairing_attack_warning` surfaces inline** as an error banner with explanatory copy, so a sender sees brute-force activity without having to read the logs.
  - **Stale-closure fix:** the LastPairHost / LastPairJoin persistence moved from inside the event listeners (where `filePath` / `codeInput` are stale from initial render) into the action functions (`startPairHost` / `startPairJoin`), matching how the classic Receive persists `LastReceive`. No more silent failures to remember.
  - **`startPairHost` / `startPairJoin` accept optional overrides** for the Resume buttons to bypass the setState round-trip.
  - **TypeScript strict mode passes**; **Vite production build clean** — Phase 4 added ~0.7 KB gzipped CSS and ~0.65 KB gzipped JS on top of Phase 3.
