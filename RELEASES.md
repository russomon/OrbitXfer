...
## v0.1.84 - 2026-06-07 — Shareable Code (pair) flow
- **The big one: a 4-word "pair code" replaces the long blob ticket as a way to start a transfer.** Tell the receiver `unhelpful-jellied-verse-uncross` in any channel — text, voice, sticky note — and they type it in. No ticket has to travel; both sides derive everything they need (sender's ephemeral NodeID, session key, blob handoff) from the code itself. Classic Send / Receive by ticket is **unchanged** and remains the default; the new mode lives behind a `Share by Ticket | Share by Code` sub-toggle on each panel.
- **How the protocol works.** New ALPN `orbitxfer/pair/1`. The sender's ephemeral NodeID is BLAKE3-derived (domain-separated) from the code, so the receiver knows where to dial without any directory service — iroh's existing relay-based discovery introduces the two peers. The handshake is **CPace** (a modern PAKE; crate `pake-cpace`) over a session key neither side knows in advance, with **Argon2id** (64 MiB × 3 iters) wrapping the code before CPace consumes it. That makes every PAKE attempt — honest or malicious — pay a ~1 second memory-hard cost. Combined with a per-source 5-attempts-per-60-s rate limit on the host, 51.7 bits of code entropy translates to roughly **74,000 years** to brute-force solo. Off-dictionary codes fail at validation; wrong-but-format-valid codes derive a different NodeID and fail at iroh discovery with **no information leakage**.
- **Post-handshake messages are AEAD-sealed** with ChaCha20-Poly1305 under the CPace session key, with per-direction counter-derived nonces. M4 (sender → receiver) carries the `TransferOffer` (iroh-blobs hash, format, total size, name, is_folder). M5 (receiver → sender) carries an optional `receiver_label` that piggybacks the v0.1.71 nickname feature, saving the dedicated label round-trip.
- **EFF Long Wordlist** sources the 4 words. The 4 hyphen-bearing entries (`drop-down`, `felt-tip`, `t-shirt`, `yo-yo`) are filtered at generation so the hyphen separator stays unambiguous — entropy loss is < 0.001 bits/word.
- **New CLI subcommands** (both files and folders work end-to-end):
  - `orbitxfer-iroh-cli pair-host <file-or-folder>` — hashes via the same `prepare_single_file` / `prepare_folder` helpers `send` uses, generates a code, registers both `PAIR_ALPN` and `iroh_blobs::ALPN` on the same Router, holds `TempTag`s for the session, waits for Ctrl+C. Multiple receivers per session, like classic Send.
  - `orbitxfer-iroh-cli pair-join <code> <output-path>` — validates and normalizes the code, derives the sender's NodeID, dials on `PAIR_ALPN`, runs the CPace handshake, decrypts the offer, then **hands off to the existing `run_receive`** to perform the actual download. Zero refactor risk to the v0.1.83 receive pipeline (preflight, retry, progress events, folder extraction, integrity checks, auto store cleanup all reused unchanged).
- **Sender UI (Share by Code).** Pick File / Pick Folder → "Generate Code" → large monospace 4-word code with a Copy button → status pill ("Awaiting receiver…" → "Receiver paired" → "Serving the blob" → "Transfer Complete") → inline NodeID, iroh-blobs hash, and format → **live pair-attempts list** that surfaces every receiver that connects, color-coded by outcome (`succeeded` / `failed` / `rejected` / `in_progress`) so brute-force activity is visible without reading the logs. A `pairing_attack_warning` event surfaces inline as an explanatory banner.
- **Receiver UI (Receive by Code).** Single text field that accepts spaces, hyphens, or mixed-case input (`Violet- Puzzle - River Cathedral` → `violet-puzzle-river-cathedral`), with a **live word-count validity indicator** (`✓ 4 words` green / `n of 4 words` muted) and an inline preview of the normalized form. Pick Destination → Receive. Status pill walks `looking_up → handshaking → offer_received` (showing the file name + size from the AEAD-sealed offer before the download starts) → `downloading → exporting → complete`. The existing `<progress>` element + bytes readout is reused — pair-join feeds the same `recvProgress` state the ticket flow does.
- **Transfer-complete summary cards** on both sides, same Name / Size / Speed / Time layout as the classic Send/Receive (plus Saved-to on the receiver). Clocks bracket only the actual data-transfer window: pair-host starts at the first `upload_started`, pair-join at `pairing_download_started` — the ~1 s Argon2id+CPace handshake is intentionally excluded so the Speed reading isn't dragged down by setup time.
- **Resume-last** for both pair flows. Sender's `↻ Re-share by code: <filename>` re-shares the same source file with a **brand-new code** (codes are always single-use). Receiver's `↻ Resume last code receive: <code>` retries the last code+destination — useful for a quick retry after a hiccup if the sender's pair-host is still running. Persisted in `localStorage` (`orbitxfer.lastPairHost.v1` / `orbitxfer.lastPairJoin.v1`), with cross-window storage-event sync.
- **Tauri sidecar plumbing.** `WindowState` grew `pair_host` / `pair_join` `CommandChild` slots; `Slot` enum grew `PairHost` / `PairJoin` variants; four new `#[tauri::command]`s (`start_pair_host` / `stop_pair_host` / `start_pair_join` / `stop_pair_join`) wired into `invoke_handler!`. `run_sidecar` teaches itself a `PairHost` branch — same per-window FsStore isolation as Send but **no** per-file identity dir (pair-host's identity is the code, not a key file). Sidecar events emit on `pair-host:*` and `pair-join:*` channels.
- **What's NOT in v0.1.84** (deferred to later): QR-code render of the 4 words for in-person pairing, a `Cmd+G` keyboard shortcut for "Generate Code", and progress events for the pair flow's hashing phase (the data flows; just no animated bar during hashing).
- **End-to-end verified locally** on this machine for both shapes before tagging:
  - Single file (512 KB) — code `latter-avenue-unworthy-refinery` — SHA-256 matched byte-for-byte (`89465ba8…`).
  - Folder with subdirectory (3 files / 448 KB) — code `railway-encrust-rehire-upon` — all files arrived at correct paths with matching SHA-256s.
  - CLI test suite: 9 / 9 pair-module unit tests passing including the slow Argon2-gated CPace round-trip.
- **No infrastructure changes.** No rendezvous server, no new relays, no schema migrations. Pure additive — every existing v0.1.83 ticket, identity, and store still works as before.
