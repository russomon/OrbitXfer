...
- **Sleep inhibitor while transferring**: when any window in the app has an active transfer, OrbitXfer holds a cross-platform wake lock so the OS doesn't put the network to sleep mid-transfer (macOS: `IOPMAssertionCreateWithName` with `kIOPMAssertionTypeNoIdleSleep`, Windows: `SetThreadExecutionState(ES_SYSTEM_REQUIRED | ES_CONTINUOUS)`, Linux: systemd-logind `Inhibit("sleep:idle", …)`). Released as soon as the last in-flight transfer finishes. A ☕ badge in the header indicates when the lock is held. Display sleep is NOT prevented — your screen can still dim.
